...
import { Plus, Trash2, Users, Save, CheckCircle } from 'lucide-react';
import type { Team, TeamMember, ProjectCategory } from '../../lib/types';

const categories: ProjectCategory[] = ['AI/ML', 'Web Development', 'Mobile App', 'IoT', 'Cybersecurity', 'Blockchain'];

export default function TeamPage() {
  const { user } = useAuthStore();
  const existingTeam = user ? db.getTeamByLeader(user.id) : null;

  const [teamName, setTeamName] = useState(existingTeam?.teamName || '');
  const [projectTitle, setProjectTitle] = useState(existingTeam?.projectTitle || '');
  const [category, setCategory] = useState<ProjectCategory>(existingTeam?.category || 'AI/ML');
  ...
  const addMember = () => {
    if (members.length >= 4) {
      setError('Maximum 4 additional members (5 total including you)');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!teamName.trim() || !projectTitle.trim()) {
      setError('Team name and project title are required');
      return;
    }
    ...
    if (existingTeam) {
      db.updateTeam(existingTeam.id, { teamName, projectTitle, category, members });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">{existingTeam ? 'Manage Team' : 'Create Team'}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button type="submit" disabled={loading}
          ...
          <Save size={18} />
          {loading ? 'Saving...' : existingTeam ? 'Update Team' : 'Create Team'}
        </button>
      </form>
    </div>
  );
