...
export default function MyEvaluationsPage() {
  ...
  const evaluations = user ? assignments
    .map(a => ({
      assignment: a,
      evaluation: db.getEvaluationByJuryProject(user.id, a.projectId),
      project: db.getProjectById(a.projectId),
    }))
    .filter(x => x.evaluation) : [];

  const avgScore = evaluations.length > 0
    ? (evaluations.reduce((s, e) => s + (e.evaluation?.totalScore || 0), 0) / evaluations.length).toFixed(1)
    : null;
    ...
    <div className="space-y-6">
      ...
      {evaluations.length === 0 ? (
        ...
        <div className="space-y-4">
          {evaluations.map(({ evaluation, project }) => evaluation && (
            <div key={evaluation.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className={`text-2xl font-bold ${
                  evaluation.totalScore >= 40 ? 'text-emerald-400' :
                  evaluation.totalScore >= 25 ? 'text-amber-400' : 'text-red-400'
                }`}>{evaluation.totalScore}<span className="text-gray-500 text-sm">/50</span></div>
              </div>
              <div className="space-y-2 mb-4">
                <ScoreBar label="Innovation" value={evaluation.innovation} />
                <ScoreBar label="Technical Implementation" value={evaluation.technical} />
                <ScoreBar label="UI/UX" value={evaluation.uiux} />
                <ScoreBar label="Presentation" value={evaluation.presentation} />
                <ScoreBar label="Impact / Usefulness" value={evaluation.impact} />
              </div>
              {evaluation.feedback && (
              ...
              <p className="text-xs text-gray-500 mt-3">Evaluated: {new Date(evaluation.evaluatedAt).toLocaleString()}</p>
            </div>
          ))}
        </div>
      )}
